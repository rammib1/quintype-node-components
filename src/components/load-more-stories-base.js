  ...
  loadMore(e) {
    ...
    this.setState({ loading: true, pageNumber: pageNumber + 1 }, () => {
      this.props.loadStories(pageNumber).then(stories => {
        this.setState({
          loading: false,
          moreStories: this.state.moreStories.concat(
            removeDuplicateStories(this.stories(), stories)
          ),
...
export class LoadMoreStoriesBase extends React.Component {
  loadMoreStories(pageNumber) {
    ...
    return getRequest(
      ...
      response => response.stories || get(response, ["results", "stories"], [])
    );
  }
